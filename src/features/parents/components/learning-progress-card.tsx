...
export const LearningProgressCard = () => {
    ...
    const difficultyStats = {
        easy: {
            ...
            avgScore: difficultyProgress.easy.reduce((acc, p) => acc + p.sequenceScore, 0) /
                (difficultyProgress.easy.length || 1),
            avgStars: difficultyProgress.easy.reduce((acc, p) => acc + p.stars, 0) /
                (difficultyProgress.easy.length || 1)
        },
        medium: {
            count: difficultyProgress.medium.length,
            avgScore: difficultyProgress.medium.reduce((acc, p) => acc + p.sequenceScore, 0) /
                (difficultyProgress.medium.length || 1),
            avgStars: difficultyProgress.medium.reduce((acc, p) => acc + p.stars, 0) /
                (difficultyProgress.medium.length || 1)
        },
        hard: {
            count: difficultyProgress.hard.length,
            avgScore: difficultyProgress.hard.reduce((acc, p) => acc + p.sequenceScore, 0) /
                (difficultyProgress.hard.length || 1),
            avgStars: difficultyProgress.hard.reduce((acc, p) => acc + p.stars, 0) /
                (difficultyProgress.hard.length || 1)
        }
    }

    const sortedEntries = [...progressEntries].sort((a, b) => b.lastPlayed - a.lastPlayed)
        ...
        <div className="flex flex-1 flex-col gap-6 p-6 pt-2">
            ...
            <div className="grid gap-6 md:grid-cols-4">
                <Card className="group hover:shadow-lg transition-all duration-300 hover:border-primary/20">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-primary">Level</CardTitle>
                        <TrendingUp className="h-4 w-4 text-primary" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{stats?.level ?? 1}</div>
                        <Progress
                            value={(stats?.currentExp ?? 0) / (stats?.nextLevelExp ?? 100) * 100}
                            className="mt-2"
                        />
                ...
                <Card className="group hover:shadow-lg transition-all duration-300 hover:border-primary/20">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-primary">Accuracy</CardTitle>
                ...
                <Card className="group hover:shadow-lg transition-all duration-300 hover:border-primary/20">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-primary">Stories</CardTitle>
                ...
                <Card className="group hover:shadow-lg transition-all duration-300 hover:border-primary/20">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-primary">Stars</CardTitle>
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <Card>
                                        <CardHeader className="pb-2">
                                            <CardTitle className="text-sm">Stories Completed</CardTitle>
                                        </CardHeader>
                                        <CardContent>
                                            <div className="text-2xl font-bold">
            ...
            <Card className="flex-1">
                <CardHeader>
                    ...
                    <CardDescription>
                        Recent story completion history
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <ScrollArea className="h-[300px] pr-4">
                        {sortedEntries.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-center p-4">
                                ...
                                <h3 className="font-semibold text-lg mb-2">No Learning History Yet</h3>
                                <p className="text-sm text-muted-foreground max-w-[250px]">
                                    When your child completes stories, their progress will appear here.
                                </p>
                            </div>
                        ) : (
                                        ...
                                        <CardContent className="p-4">
                                            ...
                                            <div className="mt-2 flex items-center gap-4 text-sm text-muted-foreground">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>
                                                        Difficulty: {entry.story?.difficulty}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <Target className="h-4 w-4" />
                                                    <span>
                                                        Attempts: {entry.sequenceAttempts}
                                                    </span>
                                                </div>
                                            </div>
                                        </CardContent>
