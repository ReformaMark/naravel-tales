    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[800px] overflow-y-auto max-h-[80vh]">
                <DialogHeader>
                    <DialogTitle className="text-2xl text-center mb-4">Choose Student</DialogTitle>
                </DialogHeader>
                <motion.div
                ...
                >
                    {students?.students.map((student) => {
                        const studentProgress = progress?.find(p => p.studentId === student._id)
                        const stars = studentProgress?.stars ?? 0
                        const completionPercentage = studentProgress ?
                            (studentProgress.sequenceScore / 100) * 100 : 0
                            ...
                            <motion.div key={student._id} variants={item}>
                                ...
                                >
                                    ...
                                    <div className="aspect-square relative flex items-center justify-center">
                                        <Avatar className={`h-24 w-24 rounded-lg ${getAvatarColor(`${student.fname} ${student.lname}`)}`}>
                                            <AvatarFallback className={`rounded-lg text-2xl font-semibold text-white ${getAvatarColor(`${student.fname} ${student.lname}`)}`}>
                                                {student.fname.charAt(0)}
                                                {student.lname.charAt(0)}
                                            </AvatarFallback>
                                        </Avatar>
                                        <div className="absolute -top-2 -right-2">
                                            <Badge variant={stars > 0 ? "default" : "secondary"} className={stars > 0 ? "bg-yellow-400 text-yellow-900" : ""}>
                                                ⭐ {stars}
                                            </Badge>
                                        </div>
                                    </div>
